...
async def async_migrate_entities_for_compatibility(hass: HomeAssistant, config_entry: ConfigEntry) -> None:
    ...
    for legacy_entity_id, sensor_suffix in LEGACY_ENTITY_MAPPINGS.items():
        ...
        if existing_entity:
            ...
            should_migrate = (
                (is_first_entry and existing_entity.config_entry_id is None) or
                (existing_entity.config_entry_id is not None and 
                 existing_entity.config_entry_id not in helen_entry_ids)
            )
    ...
    if migrated_count > 0:
        _LOGGER.info("Successfully migrated %d entities for Helen Energy integration", migrated_count)
    else:
...
def get_legacy_entity_name(sensor_type: str) -> str:
    ...
    return legacy_names.get(sensor_type, f"Helen {sensor_type.replace('_', ' ').title()}")


def should_preserve_legacy_entity_id(sensor_type: str) -> bool:
    """Check if we should try to preserve the legacy entity ID."""
    return sensor_type in [suffix for suffix in LEGACY_ENTITY_MAPPINGS.values()]
