...
A `2014-01-01` → today backfill on a real energy contract (2026-09-25): 13 chunks, the first six skipped as `no-relevant-contract`, 61,063 hours imported in 53 seconds, no artifacts at the chunk seams. Hour counts came out internally consistent — 8,760 on each full chunk, since a 365-day span crosses both DST transitions and they cancel, and 6,503 on the trailing partial chunk, one short of 271×24 for spring-forward. So `_is_out_of_contract_error` matches Helen's live 403 body, not just the shape helen-python's tests assert.

One consequence worth knowing before someone files it as a bug: **the zero-fill now begins at a chunk boundary, not at the contract start.** Helen returns a partially-overlapping chunk's pre-contract hours as `electricity=None` rather than truncating the series, so the first non-403 chunk contributes zero-filled hours for the stretch before the contract existed — in the run above, eleven months of them. Harmless (the dashboard renders zeros as no usage) but arbitrary, since the boundary drifts with leap years. Distinguishing "no contract yet" from "no consumption" needs the delivery site's true history start, which is the helen-python accessor this ADR notes as missing.
